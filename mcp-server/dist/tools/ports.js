 ...
 * 1. tcp_port_probe
 */
export async function tcpPortProbe(ip, port, timeoutMs = 2000) {
    ...
    return new Promise((resolve) => {
        ...
        let resolved = false;
        const cleanupAndResolve = (status, errorDetails) => {
            if (resolved)
                return;
            resolved = true;
            socket.destroy();
 ...
 */
export async function udpPortProbe(ip, port, timeoutMs = 1500) {
    const cleanIp = validateTarget(ip);
    const cleanPort = validatePort(port);
