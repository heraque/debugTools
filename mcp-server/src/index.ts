  ...
  try {
    switch (name) {
      ...
      case "dns_propagation_check": {
        const { hostname, record_type } = args as { hostname: string; record_type?: string };
        const result = await dnsPropagationCheck(hostname, record_type);
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
      }
      case "icmp_ping": {
        const { target, count } = args as { target: string; count?: number };
        const result = await icmpPing(target, count);
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
      }
      case "mtu_path_discovery": {
        const { target, start_size } = args as { target: string; start_size?: number };
      ...
      // L4: Ports
      case "tcp_port_probe": {
        const { ip, port, timeout_ms } = args as { ip: string; port: number; timeout_ms?: number };
        const result = await tcpPortProbe(ip, port, timeout_ms);
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
      }
      case "udp_port_probe": {
        const { ip, port } = args as { ip: string; port: number };
      ...
      case "tls_cipher_audit": {
        const { hostname, port } = args as { hostname: string; port?: number };
      ...
      case "cdn_bypass_probe": {
        const { origin_ip, domain, port } = args as { origin_ip: string; domain: string; port?: number };
      ...
      case "http_detailed_probe": {
        const { url, method, protocol_version, headers, user_agent } = args as {
          url: string;
          method?: string;
      ...
      case "grpc_health_check": {
        const { target, service_name } = args as { target: string; service_name?: string };
        const result = await grpcHealthCheck(target, service_name);
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
      }
      case "http_rate_limit_stress": {
        const { url, requests_count, concurrency } = args as { url: string; requests_count?: number; concurrency?: number };
