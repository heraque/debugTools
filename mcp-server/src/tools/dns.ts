...
async function queryResolver(dnsServer: string | null, hostname: string, recordType: string): Promise<any> {
 ...
 */
export async function dnsPropagationCheck(hostname: string, recordType = "A") {
 ...
 * 2. icmp_ping
 */
export async function icmpPing(target: string, count = 4) {
...
function parsePingOutput(stdout: string, target: string) {
  // Parse packet loss
 ...
 */
export async function mtuPathDiscovery(target: string, startSize = 1500) {
  const cleanTarget = validateTarget(target);
  ...
  const probePayloadSize = async (payloadSize: number): Promise<boolean> => {
 ...
 * 4. trace_route
 */
export async function traceRoute(target: string) {
  const cleanTarget = validateTarget(target);
 ...
 */
export async function whoisDomainLookup(domain: string) {
 ...
 * 6. bgp_asn_lookup
 */
export async function bgpAsnLookup(ip: string) {
