 ...
 */
export async function httpDetailedProbe(
  urlString: string,
  method = "GET",
...
function runHttp11Probe(url: URL, method: string, headers: Record<string, string>): Promise<any> {
...
function runHttp2Probe(url: URL, method: string, headers: Record<string, string>): Promise<any> {
  return new Promise((resolve) => {
    const isHttps = url.protocol === "https:";
    if (!isHttps) {
      resolve({
...
async function runHttp3ProbeWithCurl(url: string, method: string, headers: Record<string, string>) {
 ...
 */
export async function websocketHandshakeTest(urlString: string, headers: Record<string, string> = {}) {
 ...
 * 3. grpc_health_check
 */
export async function grpcHealthCheck(target: string, serviceName = "") {
  ...
  return new Promise((resolve) => {
    try {
      ...
      client.Check({ service: serviceName }, { deadline: Date.now() + 4000 }, (err: any, response: any) => {
        const endTime = process.hrtime.bigint();
 ...
 */
export async function httpRateLimitStress(urlString: string, requestsCount = 30, concurrency = 5) {
  const cleanUrl = validateUrl(urlString);
  ...
  const sendRequest = (): Promise<void> => {
    return new Promise((resolve) => {
      ...
      const req = lib.request(cleanUrl, { method: "GET", rejectUnauthorized: false, timeout: 5000 } as https.RequestOptions, (res) => {
