 ...
 */
function testTlsVersion(hostname: string, port: number, version: tls.SecureVersion): Promise<{ supported: boolean; cipher?: string; error?: string }> {
 ...
 * 1. tls_cipher_audit
 */
export async function tlsCipherAudit(hostname: string, port = 443) {
  const cleanHost = validateTarget(hostname);
  const cleanPort = validatePort(port);

  const versions: Record<tls.SecureVersion, string> = {
    "TLSv1": "TLS 1.0",
    "TLSv1.1": "TLS 1.1",
    "TLSv1.2": "TLS 1.2",
    "TLSv1.3": "TLS 1.3"
  ...
  await Promise.all(
    Object.entries(versions).map(async ([verKey, verName]) => {
      results[verName] = await testTlsVersion(cleanHost, cleanPort, verKey as tls.SecureVersion);
    })
  );
 ...
 */
export async function sslCertificateCheck(hostname: string, port = 443) {
  const cleanHost = validateTarget(hostname);
  const cleanPort = validatePort(port);
...
function getChainLength(cert: tls.PeerCertificate): number {
  let depth = 1;
 ...
 * 3. cdn_bypass_probe
 */
export async function cdnBypassProbe(originIp: string, domain: string, port = 443): Promise<any> {
